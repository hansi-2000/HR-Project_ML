...
if uploaded_file:
    ...
    if 'Attrition_rate' in df.columns and 'Employee_ID' in df.columns:
        ...
        # Retrieve Employee IDs of High Attrition Rate Employees
        high_risk_employees = df[df["Attrition_Category"] == "High Risk"]["Employee_ID"]
        
                # Display high-risk employees
        st.markdown(
            "<h4 style='color: red;'>High Attrition Risk Employees</h4>", 
            unsafe_allow_html=True
        )
        if not high_risk_employees.empty:
            st.write(high_risk_employees.to_frame())  # Display as table
        else:
